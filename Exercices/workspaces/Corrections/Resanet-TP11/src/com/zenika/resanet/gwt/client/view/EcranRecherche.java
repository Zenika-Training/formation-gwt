...
public class EcranRecherche extends Composite {
    ...
    private static EcranRechercheUiBinder uiBinder = GWT.create(EcranRechercheUiBinder.class);
    ...
    public EcranRecherche() {
        ...
        EcranPrincipal.LOGIN_SERVICE.findAllVilles(new AsyncCallback<Collection<Ville>>() {

            @Override
            public void onSuccess(Collection<Ville> result) {
                for (Ville ville : result) {
                    fromCityList.addItem(ville.getNom(), ville.getId().toString());
                    toCityList.addItem(ville.getNom(), ville.getId().toString());
                }
            }
    ...
    private void buildResultTable() {
        TextColumn<Transport> priceColumn = new TextColumn<Transport>() {
            ...
            public String getValue(Transport object) {
                return String.valueOf(object.getPrix());
            }
        };
        ...
        resultsTable.addColumn(availableSeatsColumn, constants.availableseats());
    ...
    public void handleSearch(ClickEvent event) {
        if (dateBox.getValue() == null) {
            ...
            EcranPrincipal.LOGIN_SERVICE.findTransportByCriteria(Long.valueOf(fromCityList.getValue(fromCityList.getSelectedIndex())),
                    Long.valueOf(toCityList.getValue(toCityList.getSelectedIndex())), dateBox.getValue(),
                    new AsyncCallback<Collection<Transport>>() {
