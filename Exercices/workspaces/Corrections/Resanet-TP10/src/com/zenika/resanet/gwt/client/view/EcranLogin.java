...
public class EcranLogin extends Composite {
	...
	public EcranLogin() {
		...
		userTextBox.getElement().setId("login");
		passwordTextBox = new PasswordTextBox();
		passwordTextBox.addStyleName("textBox");
		passwordTextBox.getElement().setId("password");
		resetButton = new Button(AppConstantes.INSTANCE.reset());
		resetButton.addStyleName("button");
		submitButton = new Button(AppConstantes.INSTANCE.log());
		submitButton.addStyleName("button");
		submitButton.getElement().setId("submitButton");

		Grid grid = new Grid(2, 2);
		grid.setWidget(0, 0, new Label(AppConstantes.INSTANCE.user()));
		grid.setWidget(1, 0, new Label(AppConstantes.INSTANCE.password()));
	...
	private void authenticateUser() {
		if (!userTextBox.getValue().isEmpty()
				&& !passwordTextBox.getValue().isEmpty()) {
			LoginDto credentials = new LoginDto(userTextBox.getValue(),
					passwordTextBox.getValue());

			EcranPrincipal.LOGIN_SERVICE.authenticate(credentials,
					new AsyncCallback<Operateur>() {

						@Override
						public void onSuccess(Operateur result) {
							EcranPrincipal.EVENT_BUS.fireEvent(new LoginEvent(
									result));
						}
						...
						public void onFailure(Throwable caught) {
							if (caught instanceof OperateurNotFoundException) {
								Window.alert(AppConstantes.INSTANCE
										.operateurnotfound());
							} else {
								Window.alert(AppConstantes.INSTANCE
										.servererror());
			...
			Window.alert(AppConstantes.INSTANCE.fillfields());
		}
	}
}
